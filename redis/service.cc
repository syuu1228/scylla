...
future<> redis_service::init(seastar::sharded<service::storage_proxy>& proxy, seastar::sharded<database>& db, seastar::sharded<auth::service>& auth_service, db::config& cfg, smp_service_group ssg)
{
    ...
    return redis::maybe_create_keyspace(cfg).then([this, &proxy, &db, ssg] {
        return _query_processor.start(std::ref(proxy), std::ref(db), ssg);
    }).then([this] {
