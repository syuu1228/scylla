...
if [ "$ID" = "ubuntu" ] || [ "$ID" = "debian" ]; then
    ...
    yum install -y yaml-cpp-devel thrift-devel antlr3-tool antlr3-C++-devel jsoncpp-devel snappy-devel systemd-devel git python sudo rpm-build python2-pystache devscripts debhelper fakeroot
elif [ "$ID" = "centos" ]; then
