...
sudo yum install -y flex bison dejagnu zlib-static glibc-static sharutils libstdc++-static gmp-devel texinfo texinfo-tex systemtap-sdt-devel mpfr-devel libmpc-devel elfutils-devel elfutils-libelf-devel glibc-devel.x86_64 glibc-devel.i686 gcc-gnat libgnat doxygen graphviz dblatex texlive-collection-latex docbook5-style-xsl python-sphinx cmake
...
do_install scylla-env-1.0-1.el7*.noarch.rpm

if [ ! -f $RPMBUILD/RPMS/x86_64/scylla-binutils-2.25-15.el7*.x86_64.rpm ]; then
...
do_install scylla-binutils-2.25-15.el7*.x86_64.rpm

if [ ! -f $RPMBUILD/RPMS/x86_64/scylla-isl-0.14-4.el7*.x86_64.rpm ]; then
...
do_install scylla-isl-0.14-4.el7*.x86_64.rpm
do_install scylla-isl-devel-0.14-4.el7*.x86_64.rpm

if [ ! -f $RPMBUILD/RPMS/x86_64/scylla-gcc-5.3.1-2.el7*.x86_64.rpm ]; then
...
if [ ! -f $RPMBUILD/RPMS/x86_64/scylla-boost-1.58.0-11.el7*.x86_64.rpm ]; then
...
do_install scylla-ragel-6.8-5.el7*.x86_64.rpm

if [ ! -f $RPMBUILD/RPMS/x86_64/scylla-gdb-7.10.1-30.el7*.x86_64.rpm ]; then
...
do_install scylla-gdb-7.10.1-30.el7*.x86_64.rpm

if [ ! -f $RPMBUILD/RPMS/noarch/python34-pyparsing-2.0.3-2.el7*.noarch.rpm ]; then
...
do_install scylla-antlr3-tool-3.5.2-1.el7*.noarch.rpm

if [ ! -f $RPMBUILD/RPMS/x86_64/scylla-antlr3-C++-devel-3.5.2-1.el7*.x86_64.rpm ];then
...
do_install scylla-antlr3-C++-devel-3.5.2-1.el7*.x86_64.rpm
