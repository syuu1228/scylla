...
MODE="release"
UNIFIED_PKG="build/release/$PRODUCT-unified-package.$(arch).tar.gz"
while [ $# -gt 0 ]; do
    case "$1" in
        "--mode")
            MODE="$2"
...
PKGS="build/$MODE/dist/tar/$PRODUCT-package.$(arch).tar.gz build/$MODE/dist/tar/$PRODUCT-python3-package.$(arch).tar.gz build/$MODE/dist/tar/$PRODUCT-jmx-package.$(arch).tar.gz build/$MODE/dist/tar/$PRODUCT-tools-package.$(arch).tar.gz"
...
for pkg in $PKGS; do
    ...
    dirname=$(basename "$pkg"| sed -e "s/-package.$(arch).tar.gz//")
