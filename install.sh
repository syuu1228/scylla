...
if [ -z "$pkg" ] || [ "$pkg" = "server" ]; then
    ...
    install -m644 dist/common/sysconfig/scylla-server -Dt "$rsysconfdir"
    ...
    for i in scylla-server scylla-housekeeping-daily scylla-housekeeping-restart; do
        if [ -f dist/debian/systemd/$i.service.d/*.conf ]; then
            install -d -m755 "$rprefix"/misc/debian/systemd/$i.service.d
            install -m644 dist/debian/systemd/$i.service.d/*.conf "$rprefix"/misc/debian/systemd/$i.service.d
        fi
